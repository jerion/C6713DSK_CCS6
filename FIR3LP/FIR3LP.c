//Using 3 lowpass coefficients with three different BW
...
{
	short i;
	dly[0] = (short)input_sample();     	 //newest input @ top of buffer

	yn = 0;                          //initialize filter output
	for (i = 0; i< N; i++)
	   yn +=(h[0][i]*dly[i]);//y(n) += h(LP#,i)*x(n-i)
